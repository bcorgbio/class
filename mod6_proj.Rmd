...
f <- list.files("data",pattern = "NH_deer",full.names = T)
deer_21 <- read_csv(f,id = "file") %>% 
  mutate(pbpNAME=
           str_replace(TOWN,"_"," ") %>% str_to_title(),
         KSM=as.numeric(KSM),
         TOTAL=as.numeric(TOTAL),
         year=gsub("NH_deer_(.*).txt","20\\1",basename(file)) %>% as.numeric()) 



nh_deer <- nh %>% left_join(deer_21) 
nh_deer %>% filter(KSM==is.na(KSM)) %>% select(pbpNAME,year,KSM) 
 
ggplot() +
  geom_sf(data=nh_deer %>% na.omit,aes(fill=KSM))+theme_void()+
  facet_wrap(.~year)+
...
map$vble <- map$MEDV
mapview(nh_deer, zcol = "KSM")

#https://www.paulamoraga.com/book-spatial/spatial-autocorrelation.html
# https://r-spatial.org/book/07-Introsf.html
#javascript:alert('Sign-in to download data')

gpw_raw <- stars::read_stars("ignore/data/land_use/NLCD_2021_Land_Cover_L48_20230630_qtPZ6zMaTAibmibqfd37.tiff")

gpw <- st_as_sf(gpw_raw)
plot(gpw_raw)

lu <- terra::rast("ignore/data/land_use/NLCD_2021_Land_Cover_L48_20230630_qtPZ6zMaTAibmibqfd37.tiff") 

lu%>% plot()

coords <- crds(lu, na.rm=FALSE)
geo_coords <- project(coords, crs(lu), "+proj=longlat")
