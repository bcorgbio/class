...
weather.d <- weather.d%>%
  ...
  left_join(sta.d%>%dplyr::select(id,name,migr.day))%>% #add the station id info (ie. name)
...
weather.wk <-weather.d %>% 
  ...
  deplyr::select(j.day,date2,name,wk.tmin,wk.tmax,wk.wvec)
...
mc.arr.weath3 <- mc.arr.weath2%>%
  group_by(year)%>%
  dplyr::mutate_at(.vars = c("wk.tmin","wk.tmax","wk.wvec"),mean)
...
Your task is to repeat data compilation and analysis for five species of your choosing from this [list of TGMs](tgms.csv). The analysis should include the following components:
  ...
  1. A `for` loop that passes your five species through `occ_data()` to retrieve occurrence data in MA during April and May in the years 2000-2019.
  2. Logistic modeling operations to predict arrival time (the Julian day when 25% of the population arrives) for each year and for each species.
...
arr.weath3 <- arr.weath2%>%
  ...
  dplyr::summarize_at(.vars = c("wk.tmin","wk.tmax","wk.wvec"),mean)%>%
  na.omit()
