...
pop_par <- bermuda2 %>% 
  ...
  ungroup %>% 
  arrange(-tot_pop) %>% 
  mutate(col=c("green","green","green","blue","blue","purple","purple","pink","pink","white","white")) %>% 
...
fix_house <- function(n,favor){
  ...
  col <- c("green","white","blue","purple","pink") 
  which_n <- which(col==favor)
  col2 <- c(sample(col[-which_n],4),col[which_n])
 prop <- sort( diff(c(0, sort(runif(4)), 1)))
prop_n <- ceiling(prop*n)
if(sum(prop_n)!=n) prop_n[5] <- prop_n[5]-diff(c(n,sum(prop_n)))
 col_ <-  sapply(1:5, function(x) rep(col2[x],prop_n[x])) %>% unlist()
}
 return(col_)
}
...
berm_house <- berm_house %>% 
  ...
  write_stars(berm_house,"data/bermudae_house_colors.tiff")
  
plot(berm_house)

t[] <- berm_house$house_color
bh <- st_rasterize(berm_house %>% st_as_sf(), ) 
  write_stars(bh,"data/bermuda_house_colors.tiff")

t <- read_stars("data/bermuda_house_colors.tiff")

plot(t)

  ggplot(aes(fill=layer))+
  geom_sf()
