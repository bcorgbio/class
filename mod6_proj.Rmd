...
snps <- setupSNP(cl_snps,2:ncol(cl_snps),sep="",)

snps <- apply(snps[,-1],2,additive) %>% data.frame() %>% na.omit()

rownames(snps) <- cl_snps %>% na.omit %>% pull(id)

pc <- prcomp(snps)

cl_pc <- pc$x[,1:5] %>% data.frame() %>% mutate(id=rownames(pc$x)) %>% left_join(cl_local)

cl_pc %>% 
  ggplot(aes(Long,Lat,col=PC2))+geom_point()
```
