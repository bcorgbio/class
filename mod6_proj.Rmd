...
1. Is there spatial heterogeneity in WTD WTD harvest (number taken during hunting season, a proxy for abundance) 
...
In our case, each variable has it's pros and cons. `pred_col` is a factor, a discrete variable with a finite set of levels (i.e. "blue", "white", etc.). One could use a logistical regression approach, but this is fraught and discrete predictors are rarely the only variable in the predictive mix. As for the proportion data with it's $>$2 categories, one could use [multinomial logistic or Dirichlet regression](https://besjournals.onlinelibrary.wiley.com/doi/10.1111/2041-210X.13234). There a paths for this in `R`, but again, they are fraught.
...
A Random Forest Regression model is powerful and accurate. It usually performs well on many problems, including features with non-linear relationships (our problem!!). Disadvantages, however, include the that there is no interpretability (kinda), overfitting may easily occur, and one must choose the number of trees to include in the model.

Let's see how we can put this into action using the `R` package `randomForest`
...
rf_dat <-  house_color %>%ungroup %>%st_drop_geometry() %>%  na.omit  %>%  dplyr::select(-name,-pred_col,-subparish)


rf <-randomForest(tot_pop~.,data=rf_dat, mtry=5, importance=TRUE, ntree=1000) 
...
#low res for plotting and handling
lu2 <- lu %>% 
 setNames("land_use") %>% 
...
nh_deer <- nh  %>% left_join(deer) %>%  dplyr::select(TOWN,HSM,PB_TOWN_16) 
 

nh_deer %>% 
  ggplot(aes(PB_TOWN_16 %>% as.numeric,HSM))+geom_point(alpha=0.3)+
  geom_smooth(method="lm") + 
  scale_x_log10()
...
nh_lu <- lu2 %>% 
  ...
  st_as_sf() %>% 
     st_intersection(nh) %>% 
  dplyr::select(land_use,TOWN)


nh_dat <- nh_lu %>% 
   as.data.frame() %>% 
   group_by(TOWN,land_use) %>% 
  count %>% 
  group_by(TOWN) %>% 
  mutate(lu_per=n/sum(n)) %>% 
  dplyr::select(-n) %>%
  pivot_wider(names_from = land_use,values_from = lu_per,values_fill = 0) %>% 
  left_join(nh_deer %>% dplyr::select(TOWN,HSM)) 

 


rf_dat <-  nh_dat %>% st_as_sf()  %>% 
  st_drop_geometry() %>% 
  ungroup%>% 
  dplyr::select(-TOWN) %>% na.omit #st_drop_geometry wont' work unles nh dat is sf
colnames(rf_dat) <- str_replace_all(string=colnames(rf_dat), pattern=" |\\,|\\/", repl="")

rf <-randomForest(HSM~.,data=rf_dat, mtry=5, importance=TRUE, ntree=1000) 

varImpPlot(rf)
#add predicitons
rf_dat$pred_HSM<- rf$predicted
