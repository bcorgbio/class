...
f_student <- gsub("_.csv","",list.files("data/shark_points"))
...
```{r rownmaes}
pca$x %>% data.frame %>% dplyr::select(Comp1,Comp2) %>% head
```
...
With that over with, we can dispense with the ugly PCA plots above and use `ggplot` to visualize our results with respect to habitat.
...
And, with a little help from the package `ggimage` we can plot images representing the extrema of the first two PCs. Notice the anterior shift of the dorsal fin along PC1 and the shortening of the caudal fin along PC2.
...
p1 + geom_image(data=PC_extremes,aes(Comp1,Comp2,image=pngs), size=.30,inherit.aes = F)+theme_classic()+xlim(range(PC_extremes$Comp1)*1.5)+ylim(range(PC_extremes$Comp2)*1.25)+theme(legend.position = c(0.1,0.2))
```
...
tax <- wormsbynames((PCA$sp %>% unique)) %>% 
  left_join(so) %>% 
  dplyr::rename(sp=valid_name) %>% 
  dplyr::select(sp,genus,family,order,superorder)

PCA <- PCA %>% 
  left_join(tax)

p1 <- PCA %>% 
  ggplot(aes(Comp1,Comp2,shape=habitat,col=order))+geom_point()



# Use map_dbl to run many models with varying value of k (centers)
tot_withinss <- map_dbl(1:5,  function(k){
  model <- kmeans(x = PCA %>% select(Comp1,Comp2,Comp3,Comp4), centers = k,nstart=50)
  model$tot.withinss
})
...
km <-  kmeans(x = PCA %>% dplyr::select(Comp1,Comp2,Comp3,Comp4), centers = 2,nstart=50)

PCA$group <- km$cluster
...
p1 + geom_image(data=PC_extremes,aes(Comp1,Comp2,image=pngs), size=.30,inherit.aes = F)+theme_classic()+xlim(range(PC_extremes$Comp1)*1.5)+ylim(range(PC_extremes$Comp2)*1.25)+theme(legend.position = c(0.1,0.2))+geom_polygon(data=hull,aes(Comp1,Comp2,lty=as.factor(group)),inherit.aes = F,alpha=0.3)


gp1 <- which(PCA$group==1)
gp2 <- which(PCA$group==2)
...
gp1_shp <- mshape(ldk_al$coords[,,gp1])
gp2_shp <- mshape(ldk_al$coords[,,gp2])


#plot the change form the concensus to min of PC1
plotRefToTarget(gp1_shp, gp2_shp, mag = 1,method="vector",label = T)
title("gp1 to gp2 warp",cex=0.6)

plotRefToTarget(gp2_shp, gp1_shp, mag = 1,method="vector",label = T)
title("gp2 to gp1 warp",cex=0.6)


gdf <- geomorph.data.frame(ldk_al,
                             morphogroup=PCA$group,
                           so=PCA$superorder,
                           habitat=PCA$habitat,
                           or=PCA$order
)

md <- morphol.disparity(coords ~ 1, groups= ~ morphogroup*or*habitat, data = gdf, iter = 10000, print.progress = FALSE)

order_disp <- md$PV.dist  %>% 
  data.frame %>% 
  mutate(from=rownames(md$PV.dist.Pval)) %>% 
  pivot_longer(-from,names_to = "to") %>% 
  mutate(order_fr=gsub("\\d\\.(.*)\\.\\w*","\\1",from),
         order_to=gsub("X\\d\\.(.*)\\.\\w*","\\1",to),
         group_fr=gsub("(\\d)\\.(.*)\\.\\w*","\\1",from),
         group_to=gsub("X(\\d)\\.(.*)\\.\\w*","\\1",to),
         hab_to=gsub("X(\\d)\\.(.*)\\.(\\w*)","\\3",to),
         hab_from=gsub("(\\d)\\.(.*)\\.(\\w*)","\\3",from)
         ) %>% 
  mutate(order=ifelse(order_fr==order_to,order_fr,NA)) %>% 
  filter(!is.na(order)) 

order_disp %>% 
  filter(order=="Carcharhiniformes") %>% 
  filter(value!=1) %>% 
  ggplot(aes(hab_to,hab_from))+
  geom_tile(aes(fill=value))+facet_grid
gp_morph<- PCA$group
names(gp_morph) <- PCA$sp


er <- compare.evol.rates(A=ldk_al$coords, phy=phy2[[1]], method="simulation",gp=gp_morph,print.progress = F)
...
**NB:** `mclapply()` works just fin on Unix-like operating systems, but not on Window. Forking---a method by which tasks are distributed to the worker cores---is not permitted in Windows
...
args2 <- args %>% mutate(pred="no") %>% 
 ...
 args %>%
  ...
  mutate(pred="yes") %>% 
  dplyr::select(sp,ntrees,time,pred)
)%>% 
...
For example, say we wanted to do something that's simple, but takes just a little bit of time, like generated 10 million random values from a normal distribution (`mean=0`, `sd=1`) any number of times. Let's encode a function that does this and allows one to specify how many times to do it. 
...
Please submit your report and all associated data to your team GitHub repository as an .Rmd document with HTML output that addresses the following:
  ...
  * Establish an array suitable for analysis using the `geomorph` functions outlined above. Construct this from all specimens digitized in [this compressed directory](data/shark_points.zip) and Procrustes align them (**note:** this will be available soon after all landmark data is collected).
  ...
  * Is the rate of morphological evolution significantly different between different habitat types?
    ...
    2. Make sure the distribution of rate estimates is created through the use of a parallelization of your analysis.
