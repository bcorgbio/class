...
This custom function `plot_SS` take three arguments, `tre` (a tree variable of class `phy`), `SS` (output from `search.shift()`), and `taxon.names` (a tibble/data.frame, containing the column `species` and `higher_taxon`). We have the higher taxon names store in our PCA tibbles (`hindwing.pca2`, etc.). The function returns a plot unter `$plot` and the `search.shift()` results under `$res`.
...
```
Notice that the clades and their taxa are color coded and the exceptional shifts are indicated by triangles (blue and down for low shifts, red and up for high shifts). Now we have a method of indicating what clades/lineags underwent a shift and the scale of the shift.
...
```
Here we just imputed our store PC vectors we used in the rate analysis and our tree of course. Now we have transformed values we can regress against one another. Let's first plot them and see what we get.
...
Wow! These PC1s Look pretty correlated! Let's see what the r-squared value is and if there is a significant relationship. For this, we need simple to create a simple linear model.
