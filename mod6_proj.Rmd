...
Now, say we wanted to visually assess how these population tiles vary with the parish geopolitical values in the the Bermuda map. For this, we need to combine the shape and population data and calculate the total population in each parish. We can start with the `bermuda_pop` `stars` object and then:
...
What's left to do is plot with `geom_sf()` as before with the `bermuda` `sf` object.
...
ss <- raster(resolution=c(0.005,0.005), crs=proj4string(br), ext=extent(br)) 
...
bermuda_grid <- bermuda2 %>% group_by(name) %>% st_make_grid(cellsize = 0.01)
...
bermuda_grid <- bermuda_pop%>% 
  setNames("population") %>% 
  st_join(bermuda_grid)
  
ggplot()+
  geom_stars(data=bermuda_grid)
...
bermuda_grid<-  bermuda_grid %>% 
  st_as_sf%>% 
  st_intersection(bermuda2) 
...
%>% 
  ...
  left_join(
    tibble(
      col=c("green","green","green","blue","blue","purple","purple","pink","pink","white","orange"),
      name=bermuda2$name
      )) %>% 
  st_drop_geometry() 
 ...
 pop_par %>% 
  group_by(name) %>% 
  st_drop_geometry() %>% 
  mutate(n_house=round(tot_pop/6,0)) %>% 
  summarise(house_color=fix_house(n=n_house,favor = col[1])) %>% 
...
```
  ...
  pivot_wider(names_from = house_col)
...
bgp %>% 
  ...
  st_drop_geometry() %>% 
  dplyr::select(subparish,house_color,population) %>% 
  group_by(subparish,house_color) %>% 
  count
