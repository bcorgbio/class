...
Please submit your report to your team GitHub repository as an .Rmd document with HTML output that addresses the following:
  ...
  * Establish an array suitable for analysis using the `geomorph` functions outlined above. Construct this from all specimens digitized in [this directory](data/class_landmark_data.zip) and Procrustes align them.
  ...
  * Is the rate of morphological evolution significantly different between different habitat types? Please be sure that
    1. This question based on a posterior distribution of rates based on all 500 phylogenetic trees.
    2. The distribution of rate estimates is create through the use of a parallelization of your analysis.
    3. You assess the statistical significance using a simple two-sided t-test (i.e. `t.test()`).
    
  * What inferences with regards to ecology and behavior can you make concerning the relationship between habitat and both shape and rates of morphological evolution? Please be sure to answer these questions based on both PC1 and PC2 shape scores.
