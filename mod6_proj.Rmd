...
forewing.gp2 <- forewing.gp %>% coo_trans(10,10)
fore.I <- sapply(1:length(forewing.gp2),function(x) forewing.gp[x]) 
...
hindwing.gp2 <- hindwing.gp %>% coo_trans(10,10)
hind.I <- sapply(1:length(hindwing.gp2),function(x) hindwing.gp[x]) 
hind.I <- sapply(hind.I, function(x) Ix(x[,1],x[,2])) %>% unlist
...
obj<-contMap(lep.tree2,fore.I2*1e10,plot=FALSE)
plot(obj,legend=0.7*max(nodeHeights(lep.tree2)),
     fsize=c(0.4,1),cex=0.1,lwd=2,res=200)
...
drops3 <- lep.tree2$tip.label[!lep.tree2$tip.label%in%sc.dat$species]
drops4 <-  sc.dat$species[!sc.dat$species%in%lep.tree2$tip.label]
lep.tree3 <- drop.tip(lep.tree2,c(drops3,drops4))
