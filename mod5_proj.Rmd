...
shark_dat <- fb_res %>% 
  left_join(shark_dat) %>% 
  filter(file_name%in% dimnames(ldk)[[3]]) %>% 
...
So, we have the image file names from whence the data, both land mark and PCA, came from. Fortunately, professor Kenaley assembled a table containin of all the the file names used for landmarking, and the species and habitat for each. You can download [this file](shark_data.csv) and read in the data.
