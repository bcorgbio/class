...
# Introduction
<center>
![](https://images.ctfassets.net/pujs1b1v0165/sTnbs2M49c9AgPZxp7rDZ/15d77d1064ae1a311ce2f04298e6928b/still_hunting_deer.jpg)
</center>
...
Now, say we wanted to visually assess how these population tiles vary with the parish geopolitical boundaries in the the Bermuda map. For this, we need to combine the shape and population data and calculate the total population in each parish. We can start with the `bermuda_pop` `stars` object and then:
  ...
  + set the name of the first (and only) layer to an intiutive name, "population", with `setNames()`
  + convert to `stars` object to an `sf` object with `st_as_sf()`
  + join the `bermuda` shape data to this, now `sf`, object with `sf_join`. This nests the raster (i.e., populaiton) data within the extents of shape (i.e., parish) data
...
Prof. Kenaley has assembled (eeeehhhem, simulated) house colors across the island according to sections of each parish, i.e. "subparish", and created an `sf` shape file you can [download here](https://bcorgbio.github.io/class/data/bermuda_shape.zip). Be sure to download and unzip the folder. I've done so to my "data" folder in my project directory. NOTE: shape files (ending in ".shp") must have a bit of associate metadata files (e.g.,".proj",".shx",".dbf") in their directory to be loaded correctly. Make sure these are there.
...
```{r houscol2}
bermuda_house <- read_sf("data/bermuda_dat.shp") 
...
So, what to do? Fortunately, we can leverage an evermore popular approach to estimating predictive power of highly correlated data: Random Forests, a relatively straight forward machine learning algorithm. 
...
**Bootstrapping**. This is the process of randomly sampling subsets of a dataset over a given number of iterations and a given number of variables and repeating an analysis based on each resample of the data. Results of each analysis are then averaged together to obtain a more powerful result than one analysis alone. Bootstrapping is a common ensemble method to infer emergent patterns in data science and an the method upon which data for RF decision trees are generated.  

RF regression models are powerful and accurate. They usually perform well on many problems, including features with non-linear relationships (our problem!!). Disadvantages, however, include the that there is no interpretability (kinda), overfitting may easily occur, and one must choose the number of variables to bootstrap and the number of trees to include in the model.

Let's see how we can put this into action using the `R` package `randomForest`. First, let' load the pakckage and assemble the data.
...
To run a random forest regression model with `randomForest`, one needs to specify a model formula, the number of trees to construct, and the number of variables to randomly sample at each split (i.e., node) in the trees. Here we say we want `tot_pop` to be predict by all the other variables (i.e., proportion of each house color) with `tot_pop~.` `mtry`, the number of varibles to sample at the splits is set to 2 and the number of trees (`ntree`) to 1000. Note that we also set `importance=T` to calculate the importance of each predictor, something we'll use later.

```{r}

rf <-randomForest(tot_pop~.,data=rf_dat, mtry=2, ntree=1000,importance=T) 
...
Let's load the data and then create another object, `lu2`, that has the sets the attribute name to "land_use", transforms the coordinate system and extent of the data to that of the `nh` `sf` object, and finally, downsamples the data 30 times, making each pixel 30 times larger. This last operation, using `st_downsample()` makes the resolution much more coarse, but eminently more manageble in terms of size and time to work with. Ah, tradeoffs.  This could take a while, but it's worth it in the end.
...
2. Intersect the `nh` object with the `lu2` object  using `st_intersection()` This will subdivide the land-use data into the `TOWN` values in the `nh` object. 
...
Please submit your report to your team GitHub repository as an .Rmd document with HTML output that addresses the following questions:
  ...
  * Is WTD spatially random, that is visually, does their abundance vary across the state?
    - In answering this question, please produce a pretty, interactive map.
  * Does land-use pattern predict WTD harvest values?
    - Please establish a data tibble bases on joining and intersecting the three data sources as outlined above.
    - Using a random forest model, assess wether land-use poportion in each town predicts deer-harvest values.
  
In anwering these questions, please also make sure to inlude 
