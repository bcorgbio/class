...
```{r morphspace, eval=FALSE, include=FALSE}

#some momocs functions are hidden, ones that make backtransformation of morphospace easy. Stored here:
source("Momocs_functions.R")


  
hind.hulls.grp <- hindwing.pca2 %>%
  group_by(higher_taxon)%>%
  slice(chull(PC1, PC2))

fore.hulls.grp <- forewing.pca2 %>%
  group_by(higher_taxon)%>%
  slice(chull(PC1, PC2))
...
hind.sp <- morphospacePCA(hindwing.pca,"PC1","PC2",pos.shp = "range",plot=F,size.shp = 0.1)[[1]]



hind.sp <- lapply(1:length(hind.sp),function(x){
  hind.sp[[x]] %>%as_tibble()%>%mutate(shp=x)}
)

hind.sp <- do.call(rbind,hind.sp)%>%dplyr::rename(x=V1,y=V2)


hind.pos <- morphospace_positions(hindwing.pca$x,nr.shp = 6,nc.shp = 5)%>%
  as_tibble()%>%dplyr::rename(pos.x=x,pos.y=y)%>%mutate(shp=1:nrow(.))


hind.sp <-hind.sp%>%
  left_join(hind.pos)%>%
  mutate(x2=x+pos.x,y2=y+pos.y)


ggplot()+geom_polygon(data=hind.sp,aes(x=x2,y=y2,group=shp),fill="white",col="darkgrey")+geom_hline(yintercept = 0, lty = 2) +geom_vline(xintercept = 0, lty = 2)+geom_point(data=hindwing.pca2,aes(x = PC1, y = PC2, color = higher_taxon),alpha = 0.5,size=4)+geom_polygon(data=hind.hulls.grp,aes(PC1,PC2,group=higher_taxon),alpha=0.01,inherit.aes = F)
```
...
This custom function `plot_SS()` takes three arguments, `tre` (a tree variable of class `phy`), `SS` (output from `search.shift()`), and `taxon.names` (a tibble/data.frame, containing the column `species` and `higher_taxon`). We have the higher taxon names store in our PCA tibbles (`hindwing.pca2`, etc.). The function returns a plot unter `$plot` and the `search.shift()` results under `$res`.
